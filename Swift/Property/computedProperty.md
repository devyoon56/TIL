...
class Person {
    ...
    func calculateBMI() -> Double {
        return weight / (height * height) * 10000       // height는 m 단위로 계산해야합니다
    }
}
...
BMI를 구하는 위의 예시를 계산 속성으로 바꾸어서 구현해봅니다.
...
위 코드에는 get 키워드만 존재하기 때문에 값을 읽어오는 것만 가능합니다. 이런 속성을 _get-only property_ 또는 _read-only property_ 라고 합니다.
...
**set** 키워드는 값을 설정한다 또는 값을 넣는다(쓴다)는 의미의 키워드입니다. 계산 속성에 값을 넣어서 set 블럭 내부의 코드를 실행할 수 있습니다. get은 값을 얻을 때 사용하지만 set은 값을 넣을 때 사용합니다. 두 가지 모두 메서드입니다.
